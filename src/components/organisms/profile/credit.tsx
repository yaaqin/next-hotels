    ...
    return (
        <>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                {isLoading ? (
                    <p className="text-xs text-gray-300 text-center py-4">{t("text.profile.credit.loading")}</p>
                ) : user?.credit ? (
                    <>
                        <p className="text-[10px] tracking-[0.14em] uppercase text-gray-400 mb-1">
                            {t("text.profile.credit.availableBalance")}
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <div>
                                <Label>{t("text.profile.credit.creditCode")}</Label>
                                <Field><span className="tracking-wider text-[13px]">{user.credit.code}</span></Field>
                            </div>
                            <div>
                                <Label>{t("text.profile.credit.validUntil")}</Label>
                        ...
                        >
                            ...
                            {t("text.profile.credit.viewHistory")}
                        </button>
                        ...
                        >
                            {t("text.profile.credit.withdraw")}
                        </button>
                    </>
                ) : (
                    <p className="text-sm text-gray-400 text-center py-4">{t("text.profile.credit.noCredit")}</p>
                )}
            </div>
                    ...
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden">
                        ...
                        {step === 'select-method' && (
                            <div className="p-6">
                                <h2 className="text-base font-semibold text-gray-900 mb-1">
                                    {t("text.profile.withdraw.selectMethodTitle")}
                                </h2>
                                <p className="text-xs text-gray-400 mb-6">
                                    {t("text.profile.withdraw.selectMethodDesc")}
                                </p>

                                <div className="flex flex-col gap-3">
                                    {/* Cash — disabled */}
                                    <button disabled className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50 opacity-40 cursor-not-allowed text-left">
                                        <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center shrink-0">
                                            <span className="text-base">💵</span>
                                        </div>
                                        <div>
                                            <p className="text-sm font-medium text-gray-700">{t("text.profile.withdraw.cashLabel")}</p>
                                            <p className="text-[11px] text-gray-400">{t("text.profile.withdraw.cashDesc")}</p>
                                        </div>
                                        <span className="ml-auto text-[10px] bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full">
                                            {t("text.profile.withdraw.soon")}
                                        </span>
                                    </button>
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-white">{t("text.profile.withdraw.cryptoLabel")}</p>
                                            <p className="text-[11px] text-gray-400">{t("text.profile.withdraw.cryptoDesc")}</p>
                                        </div>
                                ...
                                <button onClick={reset} className="w-full mt-5 text-xs text-gray-400 hover:text-gray-600">
                                    {t("text.profile.withdraw.cancel")}
                                </button>
                            </div>
                        )}
                        ...
                        {step === 'select-chain' && (
                            <div className="p-6">
                                <button onClick={() => setStep('select-method')} className="text-xs text-gray-400 hover:text-gray-600 mb-4 flex items-center gap-1">
                                    ← {t("text.profile.withdraw.back")}
                                </button>
                                <h2 className="text-base font-semibold text-gray-900 mb-1">
                                    {t("text.profile.withdraw.selectChainTitle")}
                                </h2>
                                <p className="text-xs text-gray-400 mb-6">
                                    {t("text.profile.withdraw.selectChainDesc")}
                                </p>
                                ...
                                <div className="flex flex-col gap-3">
                                    {CHAINS.map((chain) => (
                                        ...
                                        ${chain.active ? 'border-gray-900 bg-gray-900 hover:bg-gray-800' : 'border-gray-100 bg-gray-50 opacity-40 cursor-not-allowed'}`}
                                        >
                                            <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 text-base ${chain.active ? 'bg-white/10' : 'bg-gray-200'}`}>
                                            ...
                                            {chain.active && <ArrowRight01Icon size={15} className="ml-auto text-white" />}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {step === 'form' && (
                            <div className="p-6">
                                ...
                                <h2 className="text-base font-semibold text-gray-900 mb-1">
                                    {t("text.profile.withdraw.formTitle")}
                                </h2>
                                <p className="text-xs text-gray-400 mb-6">
                                    {t("text.profile.withdraw.formDesc")} <span className="font-medium text-gray-700">{formatCurrency(user.credit.balance)}</span>
                                </p>

                                {/* Amount */}
                                <div className="mb-4">
                                    <label className="text-[10px] tracking-widest uppercase text-gray-400 mb-1.5 block">
                                        {t("text.profile.withdraw.amountLabel")}
                                    </label>
                                    ...
                                    <p className="text-[10px] text-gray-400 mt-1">{t("text.profile.withdraw.amountMin")}</p>
                                </div>

                                {/* Wallet Address */}
                                <div className="mb-5">
                                    <label className="text-[10px] tracking-widest uppercase text-gray-400 mb-1.5 block">
                                        {t("text.profile.withdraw.walletLabel")}
                                    </label>
                                    <div className="relative">
                                        <input
                                            ref={walletRef}
                                        ...
                                        <button
                                            ...
                                            title={t("text.profile.withdraw.pasteTooltip")}
                                        >
                                            {pasted
                                ...
                                >
                                    {loadingPreview ? t("text.profile.withdraw.checking") : t("text.profile.withdraw.previewBtn")}
                                </button>
                        ...
                        {step === 'confirm' && preview && (
                            <div className="p-6">
                                ...
                                <h2 className="text-base font-semibold text-gray-900 text-center mb-1">
                                    {t("text.profile.withdraw.confirmTitle")}
                                </h2>
                                <p className="text-xs text-gray-400 text-center mb-6">
                                    {t("text.profile.withdraw.confirmDesc")}
                                </p>

                                <div className="bg-gray-50 rounded-xl p-4 flex flex-col gap-3 mb-5">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-gray-400">{t("text.profile.withdraw.confirmAmountLabel")}</span>
                                        <span className="text-sm font-semibold text-gray-900">{formatCurrency(preview.amountIdr)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-gray-400">{t("text.profile.withdraw.confirmReceive")}</span>
                                        <span className="text-sm font-semibold text-emerald-600">{preview.amountSgt.toFixed(6)} SGT</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-gray-400">{t("text.profile.withdraw.confirmRate")}</span>
                                        <span className="text-xs text-gray-600">{preview.rateSnapshot.toLocaleString('id-ID')}</span>
                                    </div>
                                    <Divider />
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-gray-400">{t("text.profile.withdraw.confirmBalanceBefore")}</span>
                                        <span className="text-xs text-gray-600">{formatCurrency(preview.currentBalance)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-gray-400">{t("text.profile.withdraw.confirmBalanceAfter")}</span>
                                        <span className="text-xs font-medium text-gray-800">{formatCurrency(preview.balanceAfter)}</span>
                                    </div>
                                    <Divider />
                                    <div>
                                        <span className="text-xs text-gray-400 block mb-1">{t("text.profile.withdraw.confirmWallet")}</span>
                                ...
                                <p className="text-[10px] text-amber-600 bg-amber-50 px-3 py-2 rounded-lg mb-5 text-center">
                                    {t("text.profile.withdraw.confirmWarning")}
                                </p>
