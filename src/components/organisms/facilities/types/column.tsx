...
export const facilityTypeColumns: ColumnDef<facilityTypeState>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
    cell: (info) => (
  ...
  },
  {
    ...
    header: 'Code',
    cell: (info) => (
      <span className="font-medium uppercase tracking-wide">{info.getValue() as string}</span>
    ),
  },
  {
    accessorKey: 'name',
  ...
  },
  {
    ...
    header: 'Actions',
    cell: ({ row }) => {
      const facilityType = row.original;
      return (
        <div className="flex gap-2">
          <Link
            href={`/dashboard/facility-type/${facilityType.id}`}
