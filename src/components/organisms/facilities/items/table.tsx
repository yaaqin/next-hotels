...
import { facilityItemColumns } from './column';

interface FacilityItemTableProps {
  data: facilityItemsListState[];
}

export default function FacilityItemTable({ data }: FacilityItemTableProps) {
  const [sorting, setSorting] = useState<SortingState>([]);

  const table = useReactTable({
    data,
    columns: facilityItemColumns,
    state: {
      sorting,
    },
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {table.getRowModel().rows.length > 0 ? (
              ...
              <tr>
                <td
                  colSpan={facilityItemColumns.length}
