...
import { priceProposalColumns } from './column';

interface PriceProposalTableProps {
  data: listPriceProposalState[];
}

export default function PriceProposalTable({ data }: PriceProposalTableProps) {
  const [sorting, setSorting] = useState<SortingState>([]);

  const table = useReactTable({
    data,
    columns: priceProposalColumns,
    state: { sorting },
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {table.getRowModel().rows.length > 0 ? (
              ...
              <tr>
                <td
                  colSpan={priceProposalColumns.length}
