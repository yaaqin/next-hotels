...
import { roomMaintenanceColumns } from './column';

interface RoomMaintenanceTableProps {
  data: roomMaintainenaceListState[];
}

export default function RoomMaintenanceTable({ data }: RoomMaintenanceTableProps) {
  const [sorting, setSorting] = useState<SortingState>([]);

  const table = useReactTable({
    data,
    columns: roomMaintenanceColumns,
    state: {
      sorting,
    },
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {table.getRowModel().rows.length > 0 ? (
              ...
              <tr>
                <td
                  colSpan={roomMaintenanceColumns.length}
