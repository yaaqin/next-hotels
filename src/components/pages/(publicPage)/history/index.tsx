        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="px-8 py-8 flex flex-col items-center text-center gap-5">
            <div>
              ...
              <p className="text-xs text-gray-400 leading-relaxed">Login dulu untuk melihat riwayat booking kamu.</p>
            </div>
  ...
  return (
    ...
    >
      <div className="w-full max-w-xs">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Belum ada riwayat booking</h3>
        <p className="text-sm text-gray-400 leading-relaxed mb-8">Yuk mulai cari kamar dan buat pengalaman menginap pertamamu!</p>
...
function BookingDrawer({
  ...
  return (
    <AnimatePresence>
      {booking && (
        <>
          <motion.div className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
              ...
              {booking.items?.length > 0 && (
                <div className="space-y-2">
                  ...
                  {booking.items.map((item) => {
                    const name = item.roomType?.translations?.find((t) => t.lang === "eng")?.name ?? item.roomType?.translations?.[0]?.name ?? "Room";
                    return (
              ...
              {/* Check In — tampil kalau allowCheckIn ada (true = aktif, false = disabled) */}
              ...
              {/* Check Out — tampil kalau allowCheckOut ada (true = aktif, false = disabled) */}
              {booking.allowCheckOut !== undefined && (
                <button
                  disabled={!booking.allowCheckOut}
                  onClick={() => console.log('bookingId:', booking.id)}
                  className={`w-full py-3.5 rounded-xl text-sm font-medium tracking-widest uppercase transition-all duration-200
                    ${booking.allowCheckOut
...
// ── Booking Card ───────────────────────────────────────────────────────────
// Card kembali simple — semua action ada di drawer
  ...
  return (
    ...
    >
      <div className="relative w-36 h-24 shrink-0 overflow-hidden bg-gray-100">
        {imageUrl ? (
          <img src={imageUrl} alt={roomName} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ) : (
    ...
    <div className="min-h-screen bg-gray-50/60">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-col gap-3">
          {isLoading && <div className="flex flex-col gap-3">{[...Array(3)].map((_, i) => <SkeletonCard key={i} />)}</div>}

          {error && !isLoading && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center">
              <div className="text-4xl mb-4 opacity-30">⚠️</div>
              <p className="text-sm font-medium text-gray-400">Failed to load bookings</p>
              <button onClick={() => refetch()} className="mt-4 px-5 py-2 rounded-xl border border-blue-200 text-blue-500 text-xs hover:bg-blue-50 transition">
                Try Again
              </button>
            </motion.div>
          )}
          ...
          {!isLoading && !error && bookings.length > 0 && filtered.length === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <button onClick={() => setActiveFilter("all")} className="mt-4 px-5 py-2 rounded-xl border border-gray-200 text-gray-500 text-xs hover:border-blue-300 hover:text-blue-500 transition">
                Show All
              </button>
            </motion.div>
          )}
