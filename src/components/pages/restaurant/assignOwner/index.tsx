...
import { useAssignOwner } from '@/src/hooks/mutation/adminResto/assignOwner'
...
export default function AssignOwnerPage() {
  ...
  const { mutate, isPending } = useAssignOwner()
  ...
  const validate = () => {
    ...
    if (!form.restaurantAdminId) {
      newErrors.restaurantAdminId = 'Admin resto is required'
      valid = false
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex justify-between items-center">
        <h6 className="text-xl font-semibold">Assign Owner</h6>
        <Link
